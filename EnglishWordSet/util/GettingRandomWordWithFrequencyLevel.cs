...
namespace DataAccess.util
{
    public class GettingRandomWordWithFrequencyLevel
    {
        ...
        {
            RandomWordImpl randomWordImpl = new RandomWordImpl();
            string tempWord = "";
            for (int i = 0; i < wordCountToBeGet; i++)
            {
                
                RandomWord rnword = randomWordImpl.getRandomWordWtihFrequencyList(frequencies);
                tempWord += rnword.Word + "\n";
                AddWordToDbWithFrequency(1, rnword.frequency);
                randomWordImpl.DeleteWithWord(rnword.Word);
            }
